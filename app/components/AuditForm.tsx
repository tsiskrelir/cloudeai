...
interface AuditFormProps {
  ...
  inputMode: 'url' | 'html' | 'sitemap';
  setInputMode: (v: 'url' | 'html' | 'sitemap') => void;
  loading: boolean;
  error: string;
  isDragging: boolean;
  ...
  foundSitemapUrls: string[];
  handleSitemapFetch: () => void;
  handleSitemapAnalyze: () => void;
}
...
export const AuditForm = ({
  ...
  sitemapUrl, setSitemapUrl, sitemapLoading, sitemapStatus,
  foundSitemapUrls, handleSitemapFetch, handleSitemapAnalyze,
}: AuditFormProps) => {
  const resetView = (mode: 'url' | 'html' | 'sitemap') => {
    setInputMode(mode);
    setResults(null);
    setError('');
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-6 mb-8" data-export-exclude="true">
      ...
      <div className="flex gap-3 mb-5 flex-wrap">
        <button
          onClick={() => resetView('url')}
          className="px-5 py-2.5 rounded-lg font-medium flex items-center gap-2 transition-all"
          style={inputMode === 'url' ? { backgroundColor: COLORS.primary, color: COLORS.accent } : { backgroundColor: '#f3f4f6', color: '#4b5563' }}
        >
          <Icons.Globe /> By URL
        </button>
        <button
          onClick={() => resetView('html')}
          className="px-5 py-2.5 rounded-lg font-medium flex items-center gap-2 transition-all"
          style={inputMode === 'html' ? { backgroundColor: COLORS.primary, color: COLORS.accent } : { backgroundColor: '#f3f4f6', color: '#4b5563' }}
        >
          <Icons.Code /> Paste HTML
        </button>
        <button
          onClick={() => resetView('sitemap')}
          className="px-5 py-2.5 rounded-lg font-medium flex items-center gap-2 transition-all"
          style={inputMode === 'sitemap' ? { backgroundColor: COLORS.primary, color: COLORS.accent } : { backgroundColor: '#f3f4f6', color: '#4b5563' }}
        >
          <Icons.Sitemap /> Sitemap Crawler
        </button>
      </div>
      ...
      {inputMode === 'sitemap' && (
        <div>
          <div className="flex gap-3 mb-3">
            <div className="flex-1 relative">
              <div className="absolute left-4 top-1/2 -translate-y-1/2" style={{ color: COLORS.secondary }}><Icons.Sitemap /></div>
              <input
                type="url"
                value={sitemapUrl}
                onChange={(e) => setSitemapUrl(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && !sitemapLoading && sitemapUrl && handleSitemapFetch()}
                placeholder="https://example.com/sitemap.xml"
                className="w-full pl-12 pr-4 py-3 border-2 rounded-xl outline-none"
                style={{ borderColor: sitemapUrl ? COLORS.primary : '#e5e7eb' }}
              />
            </div>
            <button
              onClick={handleSitemapFetch}
              disabled={sitemapLoading || !sitemapUrl}
              className="px-6 py-3 font-semibold rounded-xl disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center gap-2 transition-all hover:opacity-90"
              style={{ backgroundColor: COLORS.secondary, color: COLORS.primary }}
            >
              {sitemapLoading ? <Icons.Loader /> : <Icons.Search />}
              {sitemapLoading ? 'Fetching…' : 'Fetch URLs'}
            </button>
          </div>

          {sitemapStatus && (
            <div className={`mb-3 px-4 py-2 rounded-lg text-sm ${sitemapStatus.startsWith('Error') ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'}`}>
              {sitemapStatus}
            </div>
          )}

          {foundSitemapUrls.length > 0 && (
            <div className="mb-3">
              <div className="text-xs text-gray-500 mb-1">Preview (first 5 of {foundSitemapUrls.length}):</div>
              <div className="bg-gray-50 rounded-lg px-3 py-2 text-xs font-mono text-gray-700 space-y-0.5">
                {foundSitemapUrls.slice(0, 5).map((u, i) => <div key={i} className="truncate">{u}</div>)}
                {foundSitemapUrls.length > 5 && <div className="text-gray-400">…and {foundSitemapUrls.length - 5} more</div>}
              </div>
            </div>
          )}
          ...
          <div className="flex justify-between items-center">
            <p className="text-sm text-gray-500">
              Fetches URLs from your sitemap XML and bulk-audits them. Supports sitemap index files. Max 100 URLs.
            </p>
            <button
              onClick={handleSitemapAnalyze}
              disabled={loading || foundSitemapUrls.length === 0}
              className="px-8 py-3 font-semibold rounded-xl disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center gap-2 transition-all hover:opacity-90"
              style={{ backgroundColor: COLORS.accent, color: COLORS.primary }}
            >
              {loading ? <Icons.Loader /> : <Icons.Play />}
              {loading ? 'Analyzing…' : `Audit ${foundSitemapUrls.length > 0 ? foundSitemapUrls.length : ''} URLs`}
            </button>
          </div>
        </div>
      )}
